...
#img=Image.open('imagess/musicc.jpg')
st.set_page_config(page_title='Music Recommender')
image = Image.open('imagess/ok1.png')
...
df_happy = df[72000:]
#st.set_page_config(page_title='Music Recommender', page_icon=img)
# Task of function 'fun' is to take list of unique emotions & return dataframe of 30 rows.
def fun(list):

    # Creating Empty Dataframe
    data = pd.DataFrame() #pandas for seeing the multidimensional array
...
# Text or heading's
st.markdown("<h2 style='text-align: center; color: green; font-size:50px; '><b>MOOD BASED MUSIC RECOMMENDATION SYSTEM</b></h2>", unsafe_allow_html=True)

st.markdown("<h5 style='text-align: center; color: green;'><b>Know the songs According to your Mood!</b></h5>", unsafe_allow_html=True)
col1, col2, col3 = st.columns(3)

# Just for indentation
col1,col2,col3 = st.columns(3)
# check matrix type, array would have shape (3,)image2=Image.open('imagess/cute1.png')
...
with col2:
    if st.button('SCAN YOUR EMOTION!'): #button
...
# Normal text
st.markdown("<h5 style='text-align: center; color: red;'><b><i>Recommended English Songs with there link</i></b></h5>", unsafe_allow_html=True)



# Just for separation
st.write("---------------------------------------------------------------------------------------------------------------------")
image2=Image.open('imagess/cute1.png')

col1, col2, col3 = st.columns(3)
col1.image(image2, width=150)
...
try:
    ...
    for l,a,n,i in zip(new_df["link"],new_df['artist'],new_df['name'],range(30)):
        ...
        # Artist name
        st.markdown("<h5 style='text-align: center; color: brown;'><i>{}</i></h5>".format(a), unsafe_allow_html=True)
